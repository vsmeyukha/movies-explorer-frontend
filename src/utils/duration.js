...
// ? hours.toString().endsWith вот это используется стопятьсот раз, поэтому надо записать в переменную.
// ? то, что передается в hours.toString().endsWith('11'), то бишщь число, записать также в переменную. и дальше задать правила, если эта переменная в таком-то диапазоне - то выводить такое-то окончание
